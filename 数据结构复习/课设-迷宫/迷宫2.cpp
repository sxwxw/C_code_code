...
void Create1(int x,int y)//初级深度优先遍历创造(效果单一)
{
	int i;
	map[x][y] = testFlag;
	for(i = 0;i < 4;++i)//依次探索4个方向
...
void Create1Power1(int x,int y)//非递归的方式 
{
	int i;
	int xList[MAX] = {0};
	int yList[MAX] = {0};
...
void Create2(int x,int y)
{
	...
	list.rear = (list.rear + 1)%MAX;//类似循环队列，解决空间溢出 
	map[x][y] = testFlag;
...
void Create2Power1(int x,int y)
{
	...
	while(list.front != list.rear)
	{
		...
		{
			list.rear = (list.rear - 1 + MAX)%MAX ;//从队尾出元素 
...
void Initialize(int num)//这里的num就是level难度等级 
{
	int i, j;
	my_x = my_y = 1;//初始化玩家位置
	if (num % 2 == 0)
